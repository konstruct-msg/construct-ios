...
cmd_link() {
  ...
  # `&` входит в класс намеренно: за токеном идёт `&fp=<hex>` — отпечаток, который новое
  # устройство запоминает для передачи истории. Без `&` он отрезался, связывание проходило, а
  # история потом отказывала с `qr_pin_absent` («код связывания без отпечатка»).
